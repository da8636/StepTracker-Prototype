...
var chart = new Chart(ctx, {
    plugins: [{
        afterDraw: function(chartInstance) {
            ...
            chartInstance.ctx.moveTo(0, yScale.getPixelForValue(goal2));
            chartInstance.ctx.lineTo(document.getElementById('weekGraph').width, yScale.getPixelForValue(goal2));
            chartInstance.ctx.strokeStyle = "yellow";
            chartInstance.ctx.stroke();

            chartInstance.ctx.fillStyle = "yellow";
            chartInstance.ctx.fillText("Goal", 0, yScale.getPixelForValue(goal2) - 7);
        }
    }],
    type: 'bar',
    data: {
